<div class="modal-header py-3 bg-dark border-0">
    <div class="d-flex w-100">
        <h5 class="modal-title text-gray-900">{{ $content->name }}</h5>
    </div>
...
<div class="modal-body">
    <p class="text-gray-900 fw-bolder mb-0">Data:
    ...
    <p class="text-gray-900 fw-bolder mb-0">Descrição do compromisso:</p>
    <p class="text-gray-700">{{ $content->description ?? 'Sem descrição' }}</p>
    <div>
        @if ($content->members->count() > 0)
            <p class="text-gray-900 fw-bolder mb-0">Usuários:</p>
            @foreach ($content->members as $member)
                <span class="text-gray-700 fw-bold">{{ $member->information->name }}</span>
