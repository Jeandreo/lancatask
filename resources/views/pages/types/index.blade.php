            ...
            <table id="datatables" class="table table-dark-header table-striped table-row-bordered gy-2 gs-2 gx-0 border align-middle datatables no-footer">
                ...
                <tbody class="table-pd">
                    @foreach ($contents as $content)
                        <tr>
                            <td>
                                <a href="{{ route('projects.types.edit', $content->id) }}" class="text-gray-700 text-hover-primary fw-bold fs-6">
                                    {{ $content->name }}
                                </a>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="d-flex align-items-center icons-table">
                                    <a href="{{ route('projects.types.edit', $content->id) }}">
                                        <i class="fas fa-edit" title="Editar"></i>
                                    </a>
                                    <a href="{{ route('projects.types.destroy', $content->id) }}">
                                        @if ($content->status == 1)
